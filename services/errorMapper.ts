...
export const mapErrorToUserMessage = (error: any): string => {
  ...
  if (code === '42501') return 'Permission denied. Please check your login status.';
  ...
  // 4. Handle specific message patterns
  if (message) {
      if (message.includes('Failed to fetch') || message.includes('Network request failed')) {
        return 'Network connection lost. Changes are saved offline.';
      }
      if (message.includes('Quota exceeded')) return 'You have reached the storage limit.';
      if (message.includes('Permission denied')) return "You don't have permission to perform this action.";
      if (message.includes('row violates row-level security')) return 'You do not have permission to modify this item.';
      if (message.includes('value too long')) return 'Input text is too long. Please shorten it.';
      if (message.includes('Payload Too Large')) return 'Data is too large to sync.';
