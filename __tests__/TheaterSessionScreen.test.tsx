...
describe('TheaterSessionScreen', () => {
  ...
  it('should automatically flip the card after the front face is fully revealed', async () => {
    ...
    // Fast-forward through front face reveal (2 parts)
    await act(async () => { await vi.advanceTimersByTimeAsync(mockSessionData.settings.partDelay * 2); });
    expect(await screen.findByText('Aberration')).toBeInTheDocument();
  ...
  it('should sequentially reveal all parts of the back face', async () => {
    ...
    // Fast-forward to the start of the back face reveal (2 front parts + 1 flip delay + 1 back part name)
    await act(async () => { await vi.advanceTimersByTimeAsync(mockSessionData.settings.partDelay * (2 + 1 + 1)); });
  ...
  it('should transition to the next card after the cardInterval', async () => {
    ...
    // Total parts for Card 1: 2 front + 4 back = 6
    // Total time to reveal all parts: 6 * partDelay
    const timeToRevealAll = 6 * mockSessionData.settings.partDelay;
