...
import { Table, Question, VocabRow, StudySettings, StudyMode, Relation, QuestionCard, AnswerSubmission, McqPayload, TrueFalsePayload, TypingPayload, ScramblePayload, FlashcardPayload } from '../types';
...
export const createQuestion = (row: VocabRow, relation: Relation, table: Table, allRows: VocabRow[], mode: StudyMode): Question | null => {
    ...
    switch (mode) {
        ...
        case StudyMode.ClozeMCQ: {
            const placeholder = '[...]';
            // For Cloze, we assume the question text contains the placeholder.
            // If it doesn't (e.g., design-driven), this mode might need specific config.
            // Assuming standard behavior where questionColumnIds[0] has the sentence.
        ...
        case StudyMode.Scrambled: {
            // For Scramble, use the formula result if available, otherwise fallback to question cols
            ...
            const numChunks = Math.min(splitCount, totalWords);
