...
const ProgressCard: React.FC<{ 
    ...
    onResetClick: () => void,
    onViewNewWords: () => void,
}> = ({ progress, onSettingsClick, onStatsClick, onDeleteClick, onGraphClick, onResetClick, onViewNewWords }) => {
    const { handleStartConfidenceSession, handleViewConfidenceProgressContents } = useSessionStore();
    const { showToast } = useUIStore();
    const [isLoading, setIsLoading] = useState(false);

    // FIX: Changed 'learnedCards' to be the count of items in cardStates for accuracy.
    const totalCards = progress.queue.length;
    const learnedCards = Object.keys(progress.cardStates || {}).length;
        ...
        <div className="bg-surface dark:bg-secondary-800 rounded-xl shadow-lg hover:shadow-xl transition-shadow flex flex-col relative group">
            ...
            <div className="p-4 flex-grow cursor-pointer" onClick={handleStart}>
                <div className="flex items-center gap-2 mb-2">
                     <Icon name="flashcards" className="w-5 h-5 text-warning-500" />
...
const ConfidenceScreen: React.FC = () => {
    const { confidenceProgresses, deleteConfidenceProgress, resetConfidenceProgress } = useSessionDataStore();
            ...
            <main className="space-y-4">
                {confidenceProgresses.length > 0 ? (
                    ...
                    <div className="text-center py-16 bg-surface dark:bg-secondary-800/50 rounded-lg border-2 border-dashed border-secondary-200 dark:border-secondary-700">
                        <Icon name="flashcards" className="w-16 h-16 text-secondary-300 dark:text-secondary-700 mx-auto mb-4" />
