    ...
    const getDescriptionAndAction = () => {
        switch (node.type) {
            case 'table':
                ...
                const wordCount = tableData.rowCount ?? tableData.rows.length;
...
// FIX: Renamed from 'flashcard' to 'confidence' and updated type cast.
            case 'confidence':
                ...
                return { description: `${fcData.queue.length} cards`, actionLabel: "Start Session" };
            case 'queue':
                const queueData = node.data as StudyProgress;
                return { description: `${queueData.queue.length} questions in queue`, actionLabel: "View Progress" };
            case 'note':
...
const MapScreen: React.FC = () => {
    ...
    return (
        <div className="w-full h-[calc(100vh-4rem)] relative overflow-hidden bg-background dark:bg-secondary-900">
