...
export type NodeType = 'table' | 'anki' | 'confidence' | 'queue' | 'note' | 'dictation' | 'row';
...
export interface GraphNode {
    ...
    color: string;
    radius: number;
    data: Table | ConfidenceProgress | AnkiProgress | StudyProgress | Note | DictationNote | any;
    opacity?: number;
...
export const useGraphData = ({ activeFilters, searchQuery, hideOrphans, mainLinksOnly }: GraphDataParams) => {
    ...
    return useMemo(() => {
        ...
        (contextLinks || []).forEach(link => {
            // Handle Row-to-Row links (Knowledge Graph)
            if (link.sourceType === 'row') {
                // Ensure nodes exist for both rows (only if they are linked)
                // We need to find the rows efficiently.
                // This is O(Table*Row) which is heavy, but necessary. 
                // Optimization: Build a map of rowId -> {row, tableId} first? 
                // For now, let's just find them.

                // Note: We only add Row Nodes if they are part of a connection link to keep graph clean.
                // But 'allNodes' needs to be unique.

                // Helper to get-or-create node for a row
                const ensureRowNode = (id: string) => {
                    ...
                    for (const t of tables) {
                        ...
                        if (r) {
                            const label = r.cols[t.columns[0]?.id] || 'Unnamed Concept';
                            // Determine color based on relationship? Or just generic concept color.
                            allNodes.push({
                                id: r.id,
                                type: 'row',
                                label,
                                color: theme === 'dark' ? '#a8a29e' : '#d6d3d1', // Stone color
                                radius: 12,
                                data: { ...r, _tableId: t.id } // Attach table ID for context
                            });
                            return;
                        }
                    }
                ...
                ensureRowNode(link.rowId);
                ensureRowNode(link.sourceId);

                allLinks.push({
                    source: link.rowId,
                    target: link.sourceId,
                    value: 1,
                    type: 'leaf', // Use leaf style for concept links
                    // Store relationship type in link for styling? 
                    // Currently GraphLink interface doesn't support custom props easily without generic, 
                    // but we can imply from logic or extend type later.
                });

            } else {
                // Legacy: Reading/Dictation links connect to the TABLE, not the row directly (for visual cleanliness)
