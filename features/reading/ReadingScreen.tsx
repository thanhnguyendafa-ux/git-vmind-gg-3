...
const ReadingScreen: React.FC = () => {
  ...
  const handleCreateNote = async () => {
      ...
      // Find the newly created note (assuming it's the last one or we can fetch it. 
      // Current useNoteStore.createNote implementation pushes to end.
      // A better implementation would return the ID. For now, we find the newest.)
      ...
      // On mobile, close sidebar to show the new note
  ...
  // Calculate Orphan Notes for Sidebar/Browser
  const orphanNotes = React.useMemo(() => {
    const allFolderNoteIds = new Set(folders.flatMap(f => f.noteIds || []));
    // Filter out Journals for cleaner view, if desired, or keep them. 
    // The previous sidebar logic filtered them. Let's keep consistency.
  ...
  // Also filter all notes for the sidebar list (excluding journals)
  ...
  return (
    <>
        ...
        sidebar={
            ...
            onSelectNote={(id) => {
                setActiveNoteId(id);
                if (window.innerWidth < 768) setIsTablesSidebarOpen(false);
            }}
        ...
        {activeNoteId ? (
            <ReadingContent 
                noteId={activeNoteId} 
                onToggleSidebar={() => setIsTablesSidebarOpen(!isTablesSidebarOpen)}
                onBack={() => {
                    setActiveNoteId(null);
                    // On mobile, going back usually means showing the list again.
                    if (window.innerWidth < 768) {
