...
const AnkiProgressSetupScreen: React.FC = () => {
  ...
  const { setAnkiProgresses } = useSessionDataStore();
  ...
  const { availableRelations, totalRows, availableTags } = useMemo(() => {
    ...
    tables.forEach(table => {
      if (selectedTableIds.has(table.id)) {
        ...
        table.relations
          .filter(rel => (rel.tags || []).includes('Anki'))
  ...
  const handleSaveProgress = () => {
      ...
      setAnkiProgresses(prev => [...prev, newProgress]);
          ...
          <div className="animate-fadeIn">
            ...
            {availableRelations.length > 0 ? (
                ...
                <div className="text-center p-4 bg-surface dark:bg-secondary-800/50 rounded-md">
                    <p className="text-sm text-text-subtle">No relations are tagged for 'Anki' in the selected tables.</p>
                </div>
            )}
