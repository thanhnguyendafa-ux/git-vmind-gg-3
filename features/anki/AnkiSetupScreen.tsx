...
const AnkiSetupScreen: React.FC = () => {
    const ankiProgresses = useSessionDataStore(useShallow(state => state.ankiProgresses));
    const setAnkiProgresses = useSessionDataStore(state => state.setAnkiProgresses);
    const { setCurrentScreen } = useUIStore();
    ...
    const handleSaveSettings = (progressId: string, newConfig: AnkiProgress['ankiConfig']) => {
        setAnkiProgresses(prev => prev.map(p => p.id === progressId ? { ...p, ankiConfig: newConfig } : p));
        setSettingsForProgress(null);
    };
    
    const handleDelete = () => {
        if (progressToDelete) {
            setAnkiProgresses(prev => prev.filter(p => p.id !== progressToDelete.id));
