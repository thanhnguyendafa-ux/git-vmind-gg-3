...
const AnkiSessionScreen: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 bg-background dark:bg-secondary-900 flex flex-col items-center p-4 transition-colors duration-300">
            <header className="w-full max-w-2xl mb-4 flex-shrink-0">
                <div className="flex justify-between items-center text-text-subtle mb-2">
                    <span>{progress.reviewed} / {progress.total}</span>
                    <button onClick={() => handleFinishAnkiSession(sessionState)} className="hover:text-text-main dark:hover:text-secondary-100 transition-colors">End Session</button>
                </div>
                <div className="w-full bg-secondary-200 dark:bg-secondary-700 rounded-full h-2.5">
                    <div className="bg-primary-500 h-2.5 rounded-full transition-all duration-300" style={{ width: `${progress.percent}%` }}></div>
                </div>
            </header>

            <main className="flex-1 flex flex-col items-center justify-center w-full max-w-2xl overflow-hidden relative">
                <div className="w-full h-full overflow-y-auto hide-scrollbar pb-32"> {/* Padding for fixed bottom sheet */}
                    <UnifiedQuestionCard
                        card={v3Card}
                        row={currentRow}
                        table={currentTable}
            ...
            {/* Rating Interface - Adaptive (BottomSheet for Mobile / Inline for Desktop) */}
            {isAnswered && (
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-surface dark:bg-secondary-800 border-t border-secondary-200 dark:border-secondary-700 shadow-[0_-5px_20px_rgba(0,0,0,0.2)] z-50 md:static md:w-full md:max-w-2xl md:bg-transparent md:border-none md:shadow-none md:p-0 md:mt-4 animate-slideInUp">
                    
                    {lastResult !== null && v3Card.type !== 'flashcard' && (
                        <div className={`text-center font-bold mb-3 ${lastResult ? 'text-success-500' : 'text-error-500'}`}>
                        ...
                        {ratingButtons.map(btn => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`py-3 rounded-xl text-white font-bold transition-transform hover:scale-105 active:scale-95 flex flex-col items-center justify-center ${btn.color} shadow-md relative group focus:ring-2 focus:ring-offset-2 focus:ring-primary-500`}
                                >
                                    <span className="text-sm">{btn.label}</span>
                                    <span className="text-[10px] text-white/80 font-medium mt-0.5">{intervalString}</span>
                                    {/* Desktop Hotkey Hint */}
                                    <span className="hidden md:block absolute -top-2 -right-2 bg-black/50 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                        {btn.hotkey}
                                    </span>
                                </button>
                            );
             ...
             <footer className="w-full max-w-2xl mt-4 hidden md:flex items-center justify-center text-xs text-text-subtle">
                <div className="flex gap-4">
                    <span>New: {sessionState.newQueue.length}</span>
                    <span>Learning: {sessionState.learningQueue.length}</span>
                    <span>To Review: {sessionState.reviewQueue.length}</span>
                </div>
            </footer>
        </div>
    );
