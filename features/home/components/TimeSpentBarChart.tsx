  ...
  return (
    <Card className="h-full flex flex-col">
             ...
             <div className="flex bg-secondary-100 dark:bg-secondary-700/50 rounded-lg p-0.5 sm:p-1 gap-0.5 sm:gap-1 overflow-x-auto max-w-[60%] sm:max-w-none no-scrollbar">
                {(['day', 'week', 'month', 'year'] as ViewMode[]).map((mode) => (
                    <button 
                        ...
                        className={`px-2 sm:px-3 py-1 rounded-md text-[10px] font-bold transition-all uppercase tracking-wider whitespace-nowrap ${
                            viewMode === mode
        ...
        <CardContent className="p-4 sm:p-6 flex-1 flex flex-col justify-end">
            ...
            <div className="flex gap-2 sm:gap-4 h-32 sm:h-40">
                {/* Y AXIS LABELS */}
                ...
                <div className="flex-1 relative">
                    ...
                    {/* BARS */}
                    <div className="absolute inset-0 flex items-end justify-between gap-1 sm:gap-3">
                        {data.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    {/* Tooltip */}
                                    <div className={`absolute -top-10 sm:-top-12 left-1/2 -translate-x-1/2 bg-secondary-800 text-white text-[10px] sm:text-xs rounded-lg py-1 px-2 sm:px-3 whitespace-nowrap transition-all duration-200 z-20 pointer-events-none shadow-xl ${isHovered ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95'}`}>
