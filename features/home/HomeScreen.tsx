...
const QuickStatPill: React.FC<{ icon: string; label: string; value: string | number; onClick?: () => void }> = ({ icon, label, value, onClick }) => (
    <div 
        onClick={onClick}
        className={`
            flex items-center gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 rounded-xl bg-surface dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 
    ...
    return (
        <Card className="h-full flex flex-col">
            ...
            <CardContent className="p-4 flex-1 flex flex-col justify-center">
                <ActivityHeatmap activity={activity} title="" />
            </CardContent>
        </Card>
    );
};
...
const HomeScreen: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 w-full h-full overflow-y-auto overflow-x-hidden animate-fadeIn pb-32">
      <header className="mb-6 flex flex-wrap justify-between items-center gap-4">
        ...
        <div className='flex items-center gap-2'>
            ...
            {!isGuest && (
                 <button onClick={handleGuardedLogout} title="Logout" className="p-2 rounded-full text-text-subtle hover:bg-secondary-200 dark:hover:bg-secondary-700 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 sm:mb-8">
        ...
        <div className="md:col-span-2 lg:col-span-3 h-64 sm:h-80 lg:h-auto min-h-[300px] transition-transform hover:scale-[1.005] duration-500 ease-out">
            <RestorationGarden isAwake={isAwake} className="h-full w-full shadow-xl" />
        </div>
