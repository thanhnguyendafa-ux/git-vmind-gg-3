...
type Action =
  ...
  | { type: 'INITIALIZE_VIEW_SETTINGS'; payload: Partial<Pick<TableViewState, 'columnWidths' | 'rowHeight' | 'isTextWrapEnabled' | 'fontSize' | 'isBandedRows'>> & { visibleColumns?: string[] } }
...
const tableViewReducer = (state: TableViewState, action: Action): TableViewState => {
  switch (action.type) {
    ...
    case 'RESET_VIEW':
      return {
        ...state, // Keep existing widths, heights, etc.
        filters: [],
        sorts: [],
        grouping: null,
    ...
    case 'INITIALIZE_VIEW_SETTINGS':
        const { visibleColumns, ...restPayload } = action.payload;
        return { 
            ...state, 
            ...restPayload,
...
export const TableViewProvider: React.FC<TableViewProviderProps> = ({ children, columns, defaultVisibleStats }) => {
  ...
  React.useEffect(() => {
    ...
    // This will set the stats correctly on table change
