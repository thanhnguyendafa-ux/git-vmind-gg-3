            ...
            <div className="p-6">
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto">
                    {fillablePrompts.map(({ prompt, fillableCells }) => {
                        ...
                        return (
                            <div key={prompt.id} onClick={() => handleToggle(prompt.id)} className={`border rounded-lg p-3 cursor-pointer transition-all ${isSelected ? 'border-primary-500 bg-primary-500/10' : 'bg-white dark:bg-slate-800 border-slate-200/80 dark:border-slate-700/50'}`}>
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 mt-1 ${isSelected ? 'border-primary-500 bg-primary-500' : 'border-slate-300 dark:border-slate-600'}`}>
                ...
                <div className="mt-6 flex justify-end gap-2">
                    ...
                    <button onClick={() => onGenerate(selectedPromptIds)} disabled={totalSelectedToFill === 0} className="bg-primary-500 text-white font-semibold px-4 py-2 rounded-md hover:bg-primary-600 disabled:opacity-50">
