...
export const RichCell: React.FC<RichCellProps> = ({ value, column, table, rowId, isTextWrapEnabled, fontSizeClasses, editable = false }) => {
    ...
    // We wrap content in a div to handle selection visuals
    ...
    // If dragging, we might need visual feedback for "drag target range"
    // Ideally passed down via prop or calculated in parent, but here we can add a simple class if dragging over
    // Currently TableView handles the drag overlay div, so RichCell just needs to trigger updates.
    ...
    return (
        <div 
            ...
            onMouseEnter={handleMouseEnter}
        >
            <div className={`flex items-center justify-between gap-2 w-full h-full -m-1 p-1 rounded`}>
