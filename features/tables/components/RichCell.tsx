...
export const RichCell: React.FC<RichCellProps> = ({ value, column, table, rowId, isTextWrapEnabled, fontSizeClasses, editable = false }) => {
    ...
    const isVideoColumn = table.videoConfig?.videoColumnId === column.id;
    ...
    const handleSearchVideo = (e: React.MouseEvent) => {
        e.stopPropagation();
        const row = table.rows.find(r => r.id === rowId);
        if (!row) return;
        ...
        if (template) {
            ...
            // Fallback: YouTube search on first text column
            ...
            >
                <div className="relative w-full h-full flex items-center justify-center group">
                    {!value ? (
                        ...
                        <>
                            {!videoId ? (
                                <div className="w-full h-full flex items-center gap-2 text-error-500 group px-2" title="Invalid YouTube Link">
                                    <Icon name="error-circle" className="w-4 h-4 flex-shrink-0" />
                                    <span className="truncate text-xs underline decoration-dotted opacity-70 group-hover:opacity-100 transition-opacity">{value}</span>
                                </div>
                            ) : (
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="w-6 h-6 rounded-full bg-black/50 text-white flex items-center justify-center backdrop-blur-sm border border-white/20">
                                            <Icon name="play" className="w-3 h-3 ml-0.5" />
                                        </div>
                                    </div>
                ...
                {isZoomed && videoId && createPortal(
                    ...
                    >
                        ...
                        <div className="w-full max-w-4xl aspect-video bg-black rounded-lg overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                            {(() => {
                                const start = extractStartTime(value);
                                const end = extractEndTime(value);
