...
const PasteImportModal: React.FC<PasteImportModalProps> = ({ isOpen, onClose, onConfirm, pastedData, table }) => {
  ...
  const handleConfirmImport = async () => {
    ...
    if (useHeadersAsColumns) {
        ...
        // Since we aligned them by index (Col 0 -> Header 0), we map row[0] -> newColumns[0].id
              ...
              <tr>
                {headers.map((headerText, index) => (
                  <th key={index} className="p-2 border-b border-secondary-200 dark:border-secondary-700 min-w-[120px]">
                    {useHeadersAsColumns ? (
                         <div className="text-xs font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-2 py-1 rounded border border-primary-200 dark:border-primary-800">
              ...
              {(ignoreFirstLine || useHeadersAsColumns) && (
                <tr className="text-left bg-secondary-100/50 dark:bg-secondary-800/50">
                    {headers.map((header, index) => (
                        <th key={index} className="p-2 font-semibold text-text-subtle text-xs truncate border-b border-secondary-200 dark:border-secondary-700">
                            <span className="opacity-50 mr-1">Raw:</span>
                            {header}
                        </th>
                    ))}
                </tr>
