...
const FolderGridItem: React.FC<FolderGridItemProps> = ({ folder, onSelect, onDelete, onRename, onColorChange, theme, variant = 'table' }) => {
    ...
    return (
        <div 
            onClick={onSelect}
            className="group relative flex flex-col h-32 bg-surface dark:bg-secondary-800 rounded-xl border border-secondary-200 dark:border-secondary-700 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer overflow-hidden"
        >
            {/* Color Strip */}
            <div 
