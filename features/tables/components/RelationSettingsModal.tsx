...
function generateDefaultLayout(relation: Relation, table: Table, theme: Theme): RelationDesign {
    ...
    // Automatically inject a question prompt based on the answer column
    // This provides immediate context (e.g., "What is the Meaning?") instead of an empty card.
    ...
    // Back design uses answer columns by default
...
const RelationSettingsModal: React.FC<{ isOpen: boolean; onClose: () => void; onSave: (relation: Relation) => void; relation: Relation | null; table: Table; initialTab?: 'setup' | 'design'; }> = ({ isOpen, onClose, onSave, relation, table, initialTab = 'setup' }) => {
    ...
    // New State: "Live Mirror" Override
    // When set, it forces the Simulator (child) to switch to this mode immediately.
    ...
    React.useEffect(() => {
        if (isOpen && relation) {
            ...
            setForcedPreviewMode(null); // Reset override on open
            // Logic corresponds to 'setup', Design to 'design'.
    ...
    const handleToggleMode = (mode: StudyMode) => {
        handleUpdate(draft => {
            ...
            if (modes.has(mode)) {
                modes.delete(mode);
                // If unchecking the currently forced mode, stop forcing it
                if (forcedPreviewMode === mode) setForcedPreviewMode(null);
            } else {
                modes.add(mode);
                // "Live Mirror": Force preview to switch to the newly selected mode
    ...
    const handleSaveInternal = () => {
        ...
        // Auto-tagging based on selected interaction modes
        if (modes.includes(StudyMode.Flashcards)) currentTags.add('Flashcard');
        ...
        // General tag for any interactive relation
        if (modes.length > 0) {
            currentTags.add('StudySession');
        }

        const finalRelation: Relation = {
            ...editedRelation,
            tags: Array.from(currentTags),
            compatibleModes: modes, // Sync for backward compatibility
        };
    ...
    return (
        <Modal 
            isOpen={isOpen} 
            onClose={onClose} 
            title={isMobile ? (activeTab === 'logic' ? 'Logic' : 'Design') : 'Relation Editor'} 
            containerClassName={`flex flex-col p-0 overflow-hidden ${isMobile ? 'w-screen h-[100dvh] rounded-none m-0 max-h-screen' : 'w-[98vw] h-[95vh] max-w-none rounded-xl'}`}
        >
            <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
                ...
                {isMobile && (
                    <div className="flex border-b border-secondary-200 dark:border-secondary-700 flex-shrink-0 bg-surface dark:bg-secondary-800">
                        <button onClick={() => setActiveTab('logic')} className={`flex-1 py-3 text-sm font-semibold ${activeTab === 'logic' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-text-subtle'}`}>Logic</button>
                        <button onClick={() => setActiveTab('design')} className={`flex-1 py-3 text-sm font-semibold ${activeTab === 'design' ? 'text-primary-600 border-b-2 border-primary-600' : 'text-text-subtle'}`}>Design</button>
                    </div>
                )}

                {/* Left Column: Logic (Answer Definition & Settings) */}
                <div 
                    className={`flex-shrink-0 border-r border-secondary-200 dark:border-secondary-700 bg-surface dark:bg-secondary-800 overflow-y-auto transition-all duration-300 ease-in-out ${isMobile ? (activeTab !== 'logic' ? 'hidden' : 'w-full') : 'w-[400px]'}`}
                >
                    <div className="p-4 border-b border-secondary-200 dark:border-secondary-700 sticky top-0 bg-inherit z-10">
                        <label className="block text-xs font-bold text-text-subtle mb-1 uppercase">Relation Name</label>
                        <input type="text" value={editedRelation.name} onChange={(e) => handleUpdate(draft => { draft.name = e.target.value; })} className="w-full bg-secondary-100 dark:bg-secondary-700 border border-secondary-300 dark:border-secondary-600 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500" />
                    </div>
                ...
                {/* Right Column: Visual Designer (Question) */}
                <div className={`flex-1 flex flex-col min-w-0 h-full relative ${isMobile && activeTab !== 'design' ? 'hidden' : ''}`}>
                    <UnifiedDesigner 
                        table={table} 
                        relation={editedRelation} 
            ...
            <div className="p-3 bg-secondary-50 dark:bg-secondary-800/50 border-t border-secondary-200 dark:border-secondary-700 flex justify-end gap-2 flex-shrink-0 z-20">
                <Button variant="secondary" size="sm" onClick={onClose}>Cancel</Button>
                <Button size="sm" onClick={handleSaveInternal}>Save Changes</Button>
            </div>
        </Modal>
    );
};
