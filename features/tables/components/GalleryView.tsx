...
const GalleryCard: React.FC<{ row: VocabRow; table: Table; onEdit: () => void; visibleColumns: Set<string>; visibleStats: Set<string>; sortableStats: { key: string, label: string }[]; fontSize: 'sm' | 'base' | 'lg'; maxInQueue: number }> = ({ row, table, onEdit, visibleColumns, visibleStats, sortableStats, fontSize, maxInQueue }) => {
    ...
    return (
        <div onClick={onEdit} className="bg-surface dark:bg-secondary-800 rounded-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all group relative cursor-pointer flex flex-col h-full">
                    ...
                    {table.columns
                        ...
                        return (
                            <li key={col.id}>
                                ...
                                <p className={`${fontSizeClasses} text-text-main dark:text-secondary-300 truncate`}>{row.cols[col.id] || '—'}</p>
                            </li>
                        );
                    })}
                </ul>
            ...
            {visibleStats.size > 0 && (
                <div className="p-3 pt-0 border-t border-secondary-200 dark:border-secondary-700/50 mt-auto">
            ...
            <button className="absolute top-2 right-2 p-2 rounded-full bg-white/50 dark:bg-secondary-900/50 backdrop-blur-sm opacity-60 hover:opacity-100 transition-opacity" onClick={(e) => { e.stopPropagation(); onEdit(); }}>
