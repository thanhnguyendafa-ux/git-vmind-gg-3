            ...
            <div className="p-6 space-y-4">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {tableColumns.filter(c => c.id !== targetColumn.id).map(col => {
                            ...
                            return <button key={col.id} onClick={() => handleInsertPlaceholder(col.name)} className={`px-3 py-1 rounded-full text-sm font-semibold border transition-all duration-200 ease-out ${isUsed ? 'bg-primary-100 text-primary-800 border-primary-300 dark:bg-primary-900/50 dark:text-primary-300 dark:border-primary-700 ring-1 ring-primary-400' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 border-slate-300 dark:border-slate-600 hover:border-slate-400'}`}>{`{${col.name}}`}</button>;
                        })}
                    </div>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={handleSave} className="bg-primary-500 text-white font-semibold px-4 py-2 rounded-md hover:bg-primary-600">Save Prompt</button>
                    </div>
                </div>
            </div>
        </Modal>
