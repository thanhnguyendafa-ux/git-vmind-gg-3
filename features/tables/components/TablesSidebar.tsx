...
const TablesSidebar: React.FC<TablesSidebarProps> = ({ 
    ...
    const handleSelectFolderGuarded = (folderId: string | null) => {
        triggerGlobalAction(() => {
            ...
            // We also need to clear table selection which onSelectFolder typically handles or triggers via parent
            // But TableScreen logic is parent's domain.
            // Actually, if we are in TableDetail, selecting a folder means "Back to List + Select Folder".
            // So we just update the folder state. The parent component (TablesScreen) will re-render list.
            // BUT, TablesScreen also controls `selectedTableId`. If we pick a folder, we must clear table ID.
    ...
    return (
        <div className="flex flex-col h-full bg-surface dark:bg-secondary-800 overflow-y-auto custom-scrollbar">
            
            {/* Sidebar Header with Collapse Button */}
            <div className="flex items-center justify-between p-2 border-b border-secondary-200 dark:border-secondary-700">
