...
import { CardFaceDesign, Table, VocabRow, TypographyDesign, QuestionCard, DisplayTier } from '../../../../../types';
...
const CardFaceRenderer: React.FC<CardFaceRendererProps> = ({
    ...
    if (elements.length === 0 && isDesignMode) {
        return (
            <div className="p-8 text-center border-2 border-dashed border-secondary-300 dark:border-secondary-700 rounded-lg w-full group relative hover:border-primary-500 transition-colors z-20 pointer-events-auto bg-white/50 dark:bg-black/20 backdrop-blur-sm">
                            ...
                            contentNode = (
                                <div className="flex items-start gap-2 w-full">
                                    ...
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            // FIX: Use auto-detection instead of hardcoded 'en-US'
                                            const lang = detectLanguageFromText(resolvedText);
                                            playSpeech(resolvedText, lang);
                                        }}
                        ...
                        if (col) {
                            ...
                            const handlePlay = (e: React.MouseEvent) => {
                                ...
                                if (canPlayAudio) {
                                    ...
                                    const lang = audioConfigForCol?.language || detectLanguageFromText(text);
