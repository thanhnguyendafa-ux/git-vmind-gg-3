...
const TypingLayout: React.FC<TypingLayoutProps> = ({ hint, onSubmit, isImmersive, isSidebar, correctAnswerContext }) => {
  ...
  const handleCharacterSelect = (char: string) => {
      ...
      // Vmind Spec v2.6.1: Auto-dismiss canvas to return focus to input
      setIsHandwritingMode(false);
  };

  const textareaRows = isImmersive ? 4 : 2;
  // Reduced vertical padding on textarea to save space
  ...
  // Increased gap from 'gap-3' to 'gap-6' to separate the input panel (specifically candidate bar) from the bottom button
  return (
    <div className="flex flex-col w-full h-full animate-fadeIn gap-6">
      ...
      <div className="flex justify-between items-center bg-secondary-50 dark:bg-secondary-900/50 p-1.5 rounded-lg border border-secondary-200 dark:border-secondary-700/50">
          <div className="flex-1 min-w-0 px-2">
            ...
            ) : <span className="text-xs text-text-subtle italic opacity-50">Type the answer below</span>}
          </div>
        ...
        {isHandwritingMode ? (
            <div className="flex flex-col items-center gap-4 w-full">
                ...
                <div 
                    ...
                    onClick={() => setIsHandwritingMode(false)} // Click to edit manually
                ...
                {/* Expanded container for canvas - allow full width for side-by-side layout */}
                <div className="w-full flex justify-center lg:max-w-2xl mx-auto">
