    ...
    return (
        <div className={`w-full animate-slideInUp rounded-xl border p-4 shadow-lg backdrop-blur-sm ${isCorrect ? 'bg-success-50/90 border-success-200 dark:bg-success-900/20 dark:border-success-800' : 'bg-error-50/90 border-error-200 dark:bg-error-900/20 dark:border-error-800'}`}>
            ...
            <div className="flex gap-2">
                <Button variant="ghost" size="sm" className="bg-white/50 dark:bg-black/20 border border-transparent hover:border-secondary-300 dark:hover:border-secondary-600 px-3" onClick={onViewDetails}>
                    <Icon name="pencil" className="w-4 h-4" />
                </Button>
                {feedback === 'incorrect' && onViewCorrectCard && (
                    <Button variant="secondary" size="sm" onClick={onViewCorrectCard} className="px-3">
                ...
                {onNext && (
                    <Button 
                        onClick={onNext} 
                        size="sm"
                        className={`flex-1 font-bold shadow-md h-9 ${isCorrect ? 'bg-success-600 hover:bg-success-700 text-white' : 'bg-primary-600 hover:bg-primary-700 text-white'}`}
                    >
