...
import { Screen, StudySessionData, Question, StudyMode, SessionWordResult, SessionItemState, VocabRow } from '../../types';
...
const StudySessionScreen: React.FC = () => {
    ...
    }, [isAnswered, currentQuestion, itemStates, isSpeedMode, hint.text, handleNextQuestion, v3Card]);
    ...
    // Apply max-width only if NOT in Split View (Flashcard)
    const containerClass = v3Card?.type === 'flashcard' 
        ? 'w-full h-full flex flex-col' // Full width/height for split view
        : `flex-1 flex flex-col items-center justify-center w-full max-w-2xl ${shake ? 'animate-shake' : ''}`;
    ...
    return (
        <div className="fixed inset-0 bg-background dark:bg-secondary-900 flex flex-col items-center p-2 sm:p-4 transition-colors duration-300">
            ...
            {/* Only show footer hints for non-flashcard modes or standard layout */}
            {v3Card?.type !== 'flashcard' && (
                <footer className="w-full max-w-2xl flex justify-between items-center mt-4 flex-shrink-0">
