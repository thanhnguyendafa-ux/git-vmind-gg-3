...
const StudySetupScreen: React.FC = () => {
    ...
    const { setStudyProgresses } = useSessionDataStore();
  ...
  const handleCreateAndStart = () => {
    ...
    setStudyProgresses(prev => [...prev, newProgress]);
  ...
  return (
    <div className="p-4 sm:p-6 mx-auto animate-fadeIn">
                ...
                {wordSelectionMode === 'auto' ? (
                    ...
                    <div className="animate-fadeIn">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 border rounded-md p-2 bg-slate-50 dark:bg-slate-800/50">
                            {wordsForManualSelection.length > 0 ? wordsForManualSelection.map(word => { const isSelected = manualWordIds.has(word.id); return <div key={word.id} onClick={() => handleToggleManualWord(word.id)} className={`p-2 rounded-md cursor-pointer flex items-center gap-3 text-sm ${isSelected ? 'bg-emerald-100 dark:bg-emerald-900/40' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}> <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 ${isSelected ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300 dark:border-slate-600'}`}> {isSelected && <Icon name="check" className="w-3 h-3 text-white"/>} </div> <span className="truncate">{word.sourceDisplay}</span> </div> }) : <p className="text-xs text-center text-slate-500 p-4">Select relations above to see available rows.</p>}
                        </div>
                    </div>
                )}
            </div>
        ...
        <button onClick={() => setIsSaveModalOpen(true)} disabled={!isReady} className="w-full bg-emerald-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6">
