...
const Popover: React.FC<PopoverProps> = ({
  ...
  useLayoutEffect(() => {
    if (isOpen && !isMobile && triggerRef.current && contentRef.current) {
      ...
      // Temporarily show the content off-screen to measure it without causing a flicker
      ...
      if (spaceBelow >= contentRect.height || spaceBelow > spaceAbove) {
        newPos.top = triggerRect.bottom + 8; // 8px for arrow gap
  ...
  const renderDesktopPopover = () => (
    <div
      ...
      className={`fixed z-50 rounded-md shadow-lg bg-surface dark:bg-secondary-800 ring-1 ring-black ring-opacity-5 focus:outline-none 
  ...
  const renderMobileBottomSheet = () => (
    <>
      ...
      <div
        ref={contentRef}
        className={`fixed bottom-0 left-0 right-0 z-50 bg-surface dark:bg-secondary-800 rounded-t-2xl shadow-2xl flex flex-col max-h-[80vh] w-full animate-slideInUp
                   ${contentClassName}`}
        role="dialog"
        aria-modal="true"
      >
