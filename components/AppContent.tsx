...
const normalizeServerTable = (serverObj: any): Table => {
    ...
    // Metadata First: Use the exact count from the DB join if available
    // Format is usually { ..., vocab_rows: [{ count: 123 }] }
    const serverRowCount = serverObj.vocab_rows?.[0]?.count ?? serverObj.rowCount ?? 0;
...
export const AppContent: React.FC = () => {
    ...
    const { setInitialData: setTableData, fetchTablePayload } = useTableStore();
    ...
    const { isLoading, isSuccess, isError, data } = useQuery({
        ...
        queryFn: async () => {
             // Suspend sync engine to prevent race conditions during initial hydration
             const engine = VmindSyncEngine.getInstance();
             engine.suspend();
             try {
                return await fetchUserData(session!.user.id);
             } catch (e) {
                 engine.unsuspend(); // Ensure engine is unlocked if fetch fails
                 throw e;
             }
        },
    ...
    React.useEffect(() => {
        if (isSuccess && data) {
            ...
            // FIX: Remove cache-wiping logic. We do NOT save empty rows to cache here.
            // This allows offline cached data to persist until it is explicitly updated by a payload fetch.
            ...
            // --- Double-Check Hydration Protocol ---
            // Automatically verify tables that reported 0 words.
            tablesData.forEach(t => {
                // If metadata says 0 words, verify it. 
                // If metadata says > 0, the store logic will hold that number even if rows are empty initially.
                if (t.rowCount === 0) {
                     // Trigger background fetch to verify if it's truly empty or if metadata was incomplete
                     fetchTablePayload(t.id, true);
                }
            });
            // ---------------------------------------
            ...
            // Unsuspend Sync Engine now that hydration is complete
            VmindSyncEngine.getInstance().unsuspend();

            if (loading) {
                 setLoading(false);
            }
        }
    }, [isSuccess, data, setStats, setSettings, setSessionData, setTags, setTableData, setNotes, setDictationNotes, setContextLinks, setLoading, loading, fetchTablePayload]);

    React.useEffect(() => {
        if (isError) {
            // Ensure engine is unlocked if there was an error after mounting
            VmindSyncEngine.getInstance().unsuspend();
