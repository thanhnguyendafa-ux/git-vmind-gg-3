...
const ActionItem: React.FC<{ 
    ...
    // Use the shared helper to get specific details (e.g. "Word: Apple", "Dictation: Lesson 1")
        ...
        <div className={`p-3 rounded-lg border flex items-center justify-between gap-3 ${isFailed ? 'bg-error-50 border-error-200 dark:bg-error-900/10 dark:border-error-800' : 'bg-secondary-50 border-secondary-200 dark:bg-secondary-800 dark:border-secondary-700'}`}>
            ...
            <div className="flex items-center gap-2 shrink-0">
                {canEdit && isFailed && onEdit && (
                    <Button size="sm" variant="secondary" onClick={() => onEdit(action)} disabled={isProcessing} className="h-7 px-2 text-xs flex items-center gap-1">
                        <Icon name="pencil" className="w-3 h-3" /> Edit
                    </Button>
                )}
                {isFailed && (
                    <Button size="sm" onClick={handleRetry} disabled={isProcessing} className="h-7 px-2 text-xs">
                        Retry
                    </Button>
                )}
                <button onClick={handleDiscard} disabled={isProcessing} className="p-1.5 text-secondary-400 hover:text-error-500 transition-colors" title="Discard changes">
...
export const NeedsAttentionList: React.FC = () => {
    ...
    // Sort: Failed items first, then pending
    ...
    const handleSaveEdit = async (updatedRow: VocabRow) => {
        ...
        // Construct new payload preserving structure but updating row data
        const newPayload = {
        ...
        <>
            ...
            {editingAction && editingTable && (
                <WordDetailModal
                    ...
                    onDelete={() => { /* Delete not supported in fix mode, use Discard in list */ }}
                    onConfigureAI={() => { /* Config not supported in fix mode */ }}
                />
            )}
        </>
    );
