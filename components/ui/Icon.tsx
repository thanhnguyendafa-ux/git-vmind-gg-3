...
const Icon: React.FC<IconProps> = ({ name, className = 'w-6 h-6', variant = 'outline', strokeWidth = 2, title }) => {
  ...
  if (!pathData && name === 'wifi') {
      return (
        <svg xmlns="http://www.w3.org/2000/svg" fill={variant === 'filled' ? 'currentColor' : 'none'} viewBox="0 0 24 24" strokeWidth={strokeWidth} stroke="currentColor" className={className}>
