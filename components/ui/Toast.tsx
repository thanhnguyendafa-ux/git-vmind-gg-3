...
const Toast: React.FC<ToastProps> = ({ message, type = 'info', actionText, onAction, onClose, duration = 7000 }) => {
  ...
  return (
    <div
      className={`fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] sm:bottom-5 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      role={isError ? 'alert' : 'status'}
      aria-live={isError ? 'assertive' : 'polite'}
    >
      <div className={`flex items-center justify-between gap-4 py-3 px-5 rounded-xl shadow-2xl border ${styles.bg} ${styles.border}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleClose} className="p-3 -m-3 rounded-full text-secondary-200/70 hover:text-white">
